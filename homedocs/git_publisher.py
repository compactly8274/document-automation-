...
def publish(
    ...
    # Bail if we have nothing to publish
    ...
    # Sync with the remote before pushing. The local repo was created by
    # `git init` in _ensure_git_repo() and shares no history with GitHub's
    # main branch (which usually has a README commit). If the remote branch
    # exists, hard-reset to it so we get a fast-forward push. The local files
    # are always regenerated by homedocs, so a hard reset doesn't lose data.
    fetch = _run(
        ["git", "fetch", remote_url, f"refs/heads/{branch}:refs/remotes/origin/{branch}"],
        cwd=output_dir, check=False, env=env,
    )
    if fetch.returncode == 0:
        # Remote branch exists — align local with it
        reset = _run(
            ["git", "reset", "--hard", f"origin/{branch}"],
            cwd=output_dir, check=False, env=env,
        )
        if reset.returncode != 0:
            log.warning("Failed to reset to origin/%s: %s", branch, reset.stderr.strip())
    else:
        log.debug("No remote branch origin/%s yet (fetch rc=%d) — will create on push", branch, fetch.returncode)

    # Stage and commit the freshly regenerated files (the reset above wiped
    # the working tree of the previous cycle's content).
    _git(["add"] + files_to_add, cwd=output_dir)
    diff_result = _git(["diff", "--cached", "--quiet"], cwd=output_dir, check=False)
    if diff_result.returncode != 0:
        now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        _git(["commit", "-m", f"docs: regenerate {now}"], cwd=output_dir)
    else:
        log.info("Git publish: no changes to commit after reset")

    try:
        result = _run(["git", "push", remote_url, f"HEAD:{branch}"], cwd=output_dir, check=False, env=env)
        if result.returncode != 0:
            # Try setting upstream and pushing again (handles a never-pushed-before branch)
            result2 = _run(
